...
class Z extends InstancedMesh {
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 1) {
      ...
      for (let idx = 0; idx < self.count; idx++) {
        self.setColorAt(idx, colorUtils.getColorAt(idx / self.count));
        if (idx === 0) {
          this.light!.color.copy(colorUtils.getColorAt(idx / self.count));
        }
      }

      if (!self.instanceColor) return;
